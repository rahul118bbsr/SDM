 ...
 * 
 */
package main.com.sdm.server.email;
...
public class ReceiptPDFGenerator {
	...
	private void addReceiptDetails(Document doc) {
		...
		table.addCell(
				new Cell().add(sale.getProduct().getProductDescription().getDescription()).setBorder(Border.NO_BORDER))
				.setFontColor(Color.BLACK);
		String currency = " " + CurrencyHelper.fetchCurrency();
		table.addCell(new Cell().add(sale.getProduct().getProductDescription().getPrice().toString() + currency)
		...
		for (TaxTypeModel taxType : sale.getTaxModel().getTaxTypeModelList()) {
			...
			table.addCell(new Cell().add(taxType.getTaxValue().toString() + "%").setBorder(Border.NO_BORDER))
					.setFontColor(Color.BLACK);
		}
		table.addCell(new Cell().add("TOTAL").setBorder(Border.NO_BORDER)).setFontColor(Color.BLACK);
		table.addCell(
				new Cell().add(sale.getTaxModel().getProductPriceIncludingTax() + currency).setBorder(Border.NO_BORDER))
				.setFontColor(Color.BLACK);

		table.addCell(new Cell().add("AMOUNT TENDERED").setBorder(Border.NO_BORDER)).setFontColor(Color.BLACK);
		table.addCell(new Cell().add(sale.getPayment().getReceipt().getTenderedAmount() + currency)
				.setBorder(Border.NO_BORDER)).setFontColor(Color.BLACK);

		table.addCell(new Cell().add("CHANGE DUE").setBorder(Border.NO_BORDER)).setFontColor(Color.BLACK);
		table.addCell(new Cell().add(sale.getPayment().getReceipt().getBalanceAmount() + currency)
				.setBorder(Border.NO_BORDER)).setFontColor(Color.BLACK);
	...
	private void addPageBackgroundColor(PdfDocument pdfDocument, Document doc) {
		...
		canvas.rectangle(0, 0, pdfDocument.getDefaultPageSize().getWidth(),
				pdfDocument.getDefaultPageSize().getHeight());
	...
	private Image addImage(Document doc) throws MalformedURLException {
		...
		File configFile = new File(classLoader.getResource("logo.png").getFile());

		Image image = new Image(ImageDataFactory.create(configFile.getAbsolutePath()));
		doc.add(image);
		return image;
	}
	...
	private String fetchTempFileLocation() throws IOException {
		...
		pdfFileName = "OnlyTheBestVen-Rceipt-" + formattedDateTime + ".pdf";
